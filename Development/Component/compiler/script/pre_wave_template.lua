...
local function do_compile(code, env)
	...
	local f, err = load(res, '@2_inject.j', 't', env)
	if not f then
		return f, err
	end
...
local pre_wave_template = {}

function pre_wave_template:compile(op)
	log.trace("pre_wave_template compilation start.")
	local code, err = io.load(op.input)
	if not code then
		log.error("pre_wave_template read " .. op.input .. ". Error: " .. err)
		return false
	end
	...
	if not ok then
		...
		log.error("pre_wave_template error processing: " .. tostring(res))
		return false
	end

	local ok, err = io.save(op.output, res)
	if not ok then
		log.error("pre_wave_template write " .. op.output .. ". Error: " .. err)
		return false
	end
	
	log.debug("pre_wave_template compilation succeeded.")
	return true
end

return pre_wave_template
