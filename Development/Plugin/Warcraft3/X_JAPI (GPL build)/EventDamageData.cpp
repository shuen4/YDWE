...
uintptr_t searchUnitDamageFunc_ptr()
{
    return get_war3_searcher().search_int_in_rdata(getUnitDamageFunc());
    war3_searcher& s = get_war3_searcher();
    uintptr_t str = s.search_string_ptr("EtherealDamageBonusAlly", sizeof("EtherealDamageBonusAlly"));
...
uintptr_t getUnitDamageFunc_ptr()
{
    static uintptr_t s_result = searchUnitDamageFunc_ptr();
    return s_result;
}
...
uint32_t __cdecl X_TriggerRegisterPlayerUnitDamagedEvent(jass::jhandle_t trigger, jass::jhandle_t player) {
    return TriggerEvent::TriggerRegisterPlayerUnitEvent(trigger, player, EVENT_PLAYER_UNIT_DAMAGED, NULL);
}

uint32_t __cdecl X_TriggerRegisterPlayerUnitDamagingEvent(jass::jhandle_t trigger, jass::jhandle_t player) {
    return TriggerEvent::TriggerRegisterPlayerUnitEvent(trigger, player, EVENT_PLAYER_UNIT_DAMAGING, NULL);
}

uint32_t __cdecl X_SetEventAttackType(uint32_t type) {
...
uint32_t __cdecl X_SetEventDamageType(uint32_t type) {
...
uint32_t __cdecl X_SetEventWeaponType(uint32_t type) {
...
void InitializeUnitEvent_EventDamageData() {
    ...
    uint32_t ptr = getUnitDamageFunc_ptr();
    WriteMemory(ptr, getUnitDamageFunc());
    ...
    jass::japi_add((uintptr_t)X_TriggerRegisterPlayerUnitDamagedEvent,  "X_TriggerRegisterPlayerUnitDamagedEvent",  "(Htrigger;Hplayer;)Hevent;");

    // 玩家单位接受伤害事件 (计算护甲前)
    jass::japi_add((uintptr_t)X_TriggerRegisterPlayerUnitDamagingEvent, "X_TriggerRegisterPlayerUnitDamagingEvent", "(Htrigger;Hplayer;)Hevent;");
    jass::japi_add((uintptr_t)X_SetEventAttackType,                     "X_SetEventAttackType",                     "(Hattacktype;)B");
    jass::japi_add((uintptr_t)X_SetEventDamageType,                     "X_SetEventDamageType",                     "(Hdamagetype;)B");
    jass::japi_add((uintptr_t)X_SetEventWeaponType,                     "X_SetEventWeaponType",                     "(Hweapontype;)B");
}
}
