...
defineEventData(JAPI_PlayerUnitAttackLaunchEventData, TriggerEvent::EVENT_PLAYER_UNIT_ATTACK_LAUNCH, 0, uint32_t weapon_index; uint32_t target_x; uint32_t target_y; uint32_t src_x; uint32_t src_y; bool isPointTarget; bool exposeSource;);
...
uint32_t __fastcall fake_CAbilityAttack_AttackTarget(uint32_t _this, uint32_t, uint32_t target /* widget */, uint32_t weapon_index, uint32_t a4, uint32_t a5, uint32_t a6, uint32_t exposeSource) {
    ...
    if (TriggerEvent::IsEventRegistered(TriggerEvent::GetUnitOwner(pUnit), TriggerEvent::EVENT_PLAYER_UNIT_ATTACK_LAUNCH)) {
        ...
        pPlayerUnitAttackLaunchEventData->exposeSource = exposeSource;
        ...
        return base::this_call<uint32_t>(real_CAbilityAttack_AttackTarget, _this, target, weapon_index, a4, a5, a6, (uint32_t)pPlayerUnitAttackLaunchEventData->exposeSource);
    }
