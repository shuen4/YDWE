...
#define do_if(typeID, vfOff, ...) case typeID: base::this_call_vf<void>(pBuff, vfOff, __VA_ARGS__); break
        switch (typeID) {
            ...
            do_if('Bgra', 0x354, pTargetUnit, duration, pSrcUnit, *data1, *data2, *data3);
            // BNef
            // do_if('BNef', 0x354, pTargetUnit, duration, pAbility);
            // 未知(0) 空中单位坠落时间 空中单位高度 近战攻击范围
            ...
            // 分裂次数(整数), 分裂延迟, 分裂所需攻击次数(整数), 最大生命值参数, 分裂生命周期奖励, 分裂次数(整数), 剩余分裂次数(整数) 分裂距离
        ...
        case 'Bfro': // 移速增加(%) 攻速增加(%)
            ...
            // BIpd
            // 无数据
            do_if('BIcb', 0x31C, pTargetUnit, duration);
            // 无数据
            ...
            // Bmlc
            // Bmfa
            // 影响范围 每点魔法造成伤害(单位) 每点魔法造成伤害(英雄) 最大伤害(单位) 最大伤害(英雄) 施法距离 魔法施法时间 仅溅射伤害有魔法单位(整数) 目标允许(整数) 命令id(852512) 绑定buff(整数)
            ...
            // 施加伤害(%) 所受伤害(%)
            do_if('BOmi', 0x364, pTargetUnit, duration, data1, data2, pSrcUnit);
            // 施加伤害(%) 所受伤害(%)
            do_if('BIil', 0x364, pTargetUnit, duration, data1, data2, pSrcUnit);
            ...
            // BOww
            // 转变时间 移速增加(%) 攻击伤害加成 AOE(有什么用?) 绑定技能(整数) 技能ID(整数) 启用加成伤害(整数)
